...
for (k in 1:length(variables)) {
  ...
  for (subset in subsets) {
    ...
    rf_grid <- expand.grid(mtry = seq(2, ncol(training) - 1, by = 1))
    
    # Utilizzo di un modello per addestrare i dati con il metodo Random Forest
    rf_model <- train(
      result ~ ., data = train_data,
      method = "rf",
      tuneGrid=rf_grid,
      trControl = trainControl(method = "cv", number = 5),
      ntree=100 #Specifica il numero di alberi
    )
    ...
    if (accuracy > temp_accuracy) {
      ...
      temp_subset <- subset
      temp_model <- rf_model  # Aggiungi il modello corrente
